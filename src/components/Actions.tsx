...
export function OpenInFinderAction(props: { path: string }) {
  return <Action.Open title="Open in Finder" target={"file://" + props.path} />;
}
...
function OpenInXcode(props: { path: string }) {
  ...
  return <Action.Open title={`Open ${xcodeProps.type} in Xcode`} target={xcodeProps.projectPath} application="Xcode" />;
}
...
function determineProjectType(projectPath: string): OpenInIDEProps {
  ...
  if (
    entries.some((entry) => entry.endsWith(".xcodeproj") || entry.endsWith(".xcworkspace") || entry === "Package.swift")
  ) {
    return {
  ...
  if (
    entries.some(
      (entry) =>
        ...
        entry === "settings.gradle" ||
        entry === "settings.gradle.kts",
    )
  ) {
    return {
  ...
  if (
    entries.includes(".idea") ||
    entries.includes("pom.xml") ||
    entries.some((entry) => entry === "build.gradle" || entry === "build.gradle.kts")
  ) {
    return {
